<?php
/* ============================================================================
   ingesta.php — descarga, archiva y carga las listas del SAT.
...
for ($i = 0; $i < count($args); $i++) {
    if ($args[$i] === '--grupo') { $grupo = $args[$i + 1] ?? null; $i++; }
    elseif ($args[$i][0] !== '-') $soloUna = $args[$i];
}

/* ------------------------------------------------------------- descubrimiento */
$d = fuentes_descubrir();
if (!$d['ok']) { fwrite(STDERR, "FALLO: {$d['motivo']}\n"); exit(2); }

$listas = $d['listas'];
if ($soloUna) $listas = array_intersect_key($listas, [$soloUna => 1]);
if ($grupo)   $listas = array_filter($listas, fn($l) => $l['grupo'] === $grupo);
if (!$listas) { fwrite(STDERR, "Ninguna lista coincide con lo pedido.\n"); exit(2); }
...
echo "\n" . ($errores ? "Terminado con $errores fallo(s). " : "Terminado. ")
   . "Eventos nuevos: $totalEventos\n";
exit($errores ? 1 : 0);
...
/* ==========================================================================
   ...
   ========================================================================== */
function ingerir(string $clave, array $lista, bool $forzar): array
{
    ...
    printf("   descargado  %s bytes  sha %s…\n", number_format($bytes), substr($sha, 0, 12));
    ...
    fclose($h);
    unlink($tmp);
    printf("   filas %s · cargadas %s · suprimidas %d · descartadas %d\n",
    ...
    printf("   altas %d · cambios %d · bajas %d%s\n",
